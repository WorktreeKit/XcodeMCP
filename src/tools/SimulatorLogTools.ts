...
export class SimulatorLogTools {
  private static readonly sessions = new Map<string, LogSession>();

  public static async startLogCapture(params: {
    ...
    try {
      if (captureConsole) {
        ...
        consoleProcess.stdout?.on('data', (chunk: Buffer) => {
          logStream.write(`[console] ${chunk.toString()}`);
        });
        consoleProcess.stderr?.on('data', (chunk: Buffer) => {
      ...
      osLogProcess.stdout?.on('data', (chunk: Buffer) => {
        logStream.write(chunk);
      });
      osLogProcess.stderr?.on('data', (chunk: Buffer) => {
  ...
  public static async stopLogCapture(sessionId: string): Promise<McpResult> {
