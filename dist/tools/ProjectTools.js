...
export class ProjectTools {
    static async ensureXcodeIsRunning() {
    ...
    `;
        ...
        try {
            ...
            const xcodeSelectResult = await new Promise((resolve, reject) => {
    ...
    static async waitForProjectToLoad(projectPath, maxRetries = 30, retryDelayMs = 1000) {
        const checkScript = `
      (function() {
        try {
    ...
    static async openProjectAndWaitForLoad(projectPath) {
    ...
    static async closeProject(projectPath) {
     ...
     */
    static async getTestTargets(projectPath) {
        try {
            ...
            const getHumanReadableProductType = (productType) => {
                switch (productType) {
