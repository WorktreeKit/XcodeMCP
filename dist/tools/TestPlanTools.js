...
export class TestPlanTools {
     ...
     */
    static async updateTestPlan(testPlanPath, testTargets) {
        try {
            ...
            try {
                ...
                throw new McpError(ErrorCode.InvalidParams, `Failed to read test plan file: ${testPlanPath}. Error: ${error instanceof Error ? error.message : String(error)}`);
            }
            const splitIdentifier = (identifier, targetName) => {
            ...
            const normalizedTargets = testTargets.map(config => {
                ...
                const normalizedSelected = (config.selectedTests || []).map(identifier => splitIdentifier(identifier, targetName));
                const normalizedSkipped = (config.skippedTests || []).map(identifier => splitIdentifier(identifier, targetName));
                return {
                    config,
                    normalizedSelected,
                    normalizedSkipped
            ...
            try {
                ...
                throw new McpError(ErrorCode.InternalError, `Failed to write test plan file: ${testPlanPath}. Error: ${error instanceof Error ? error.message : String(error)}`);
            }
            const selectedTestCount = testTargets.reduce((count, target) => count + (target.selectedTests?.length || 0), 0);
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to update test plan: ${errorMessage}`);
        }
    }
    /**
     * Update test plan and automatically trigger lightweight reload
     */
    static async updateTestPlanAndReload(testPlanPath, projectPath, testTargets) {
        try {
            ...
            throw new McpError(ErrorCode.InternalError, `Failed to update test plan and reload: ${errorMessage}`);
        }
    }
    /**
     * Attempt to trigger Xcode to reload test plan without closing project
     */
    static async triggerTestPlanReload(testPlanPath, projectPath) {
        try {
     ...
     */
    static async scanAvailableTests(projectPath, testPlanPath) {
        try {
            const { exec } = await import('child_process');
            const { promisify } = await import('util');
            const execAsync = promisify(exec);
            ...
            if (testPlanPath) {
                try {
                    ...
                    if (testPlan.testTargets && testPlan.testTargets.length > 0) {
                        ...
                        testPlan.testTargets.forEach((target, index) => {
                            ...
                            if (target.selectedTests && target.selectedTests.length > 0) {
                                ...
                                target.selectedTests.slice(0, 3).forEach((test) => {
                                    message += `      • ${test}\n`;
                                });
...
//# sourceMappingURL=TestPlanTools.js.map