...
export class XCResultParser {
     ...
     */
    static isXCResultReadable(xcresultPath) {
     ...
     */
    static async waitForXCResultReadiness(xcresultPath, testDurationMs = 1200000) {
     ...
     */
    async formatTestResultsSummary(includeIndividualTests = false, maxPassedTests = 5) {
     ...
     */
    async getTestActivities(testId) {
        try {
            const output = await XCResultParser.runXCResultTool([
     ...
     */
    async getTestAttachments(testId) {
        try {
     ...
     */
    async exportAttachment(attachmentId, filename) {
     ...
     */
    async findTestNode(testIdOrIndex) {
     ...
     */
    async formatTestDetails(testIdOrIndex, includeConsole = false) {
        const analysis = await this.analyzeXCResult();
    ...
    static async runXCResultTool(args, timeoutMs = 15000) {
    ...
    formatTestActivities(jsonString) {
        try {
    ...
    formatActivity(activity, baseTime, indent, activities) {
        if (!activity.title)
            return;
    ...
    formatTestHierarchy(node, prefix, startIndex) {
        let output = '';
    ...
    findTestNodeByIndex(nodes, targetIndex) {
        let currentIndex = 1;
        const search = (nodes) => {
            for (const node of nodes) {
                if (node.nodeType === 'Test Case') {
     ...
     */
    extractAttachmentsFromActivities(json, attachments, parentTimestamp, testStartTime) {
        if (!json)
            return;
...
//# sourceMappingURL=XCResultParser.js.map